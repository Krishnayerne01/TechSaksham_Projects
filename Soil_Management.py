...
from PyQt6.QtWidgets import QApplication, QMainWindow, QPushButton, QVBoxLayout, QWidget, QMessageBox, QTableWidget, QTableWidgetItem, QHBoxLayout, QLabel
...
class SoilHealthApp(QMainWindow):
    def __init__(self):
        ...
        self.insert_button.setStyleSheet("background-color: #4CAF50; color: white; font-size: 14px;")
        self.insert_button.clicked.connect(self.insert_data)
        button_layout.addWidget(self.insert_button)

        self.export_button = QPushButton("📄 Export Data as CSV")
        self.export_button.setStyleSheet("background-color: #2196F3; color: white; font-size: 14px;")
        self.export_button.clicked.connect(self.export_csv)
        button_layout.addWidget(self.export_button)

        self.graph_button = QPushButton("📊 Show Graph")
        self.graph_button.setStyleSheet("background-color: #FF9800; color: white; font-size: 14px;")
        self.graph_button.clicked.connect(self.update_graph)
        button_layout.addWidget(self.graph_button)
        
        self.refresh_button = QPushButton("🔄 Refresh Data")
        self.refresh_button.setStyleSheet("background-color: #9C27B0; color: white; font-size: 14px;")
        self.refresh_button.clicked.connect(self.load_data)
        button_layout.addWidget(self.refresh_button)
        
        self.delete_button = QPushButton("🗑️ Delete Selected Data")
        self.delete_button.setStyleSheet("background-color: #F44336; color: white; font-size: 14px;")
        self.delete_button.clicked.connect(self.delete_data)
        button_layout.addWidget(self.delete_button)

        self.delete_all_button = QPushButton("🗑️ Delete All Records")
        self.delete_all_button.setStyleSheet("background-color: #E91E63; color: white; font-size: 14px;")
